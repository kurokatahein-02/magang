...
const ControlKegiatanISP = () => {
  // --- STATE MANAGEMENT ---
  const [view, setView] = useState('table'); 
  const [isExpanded, setIsExpanded] = useState(false);
  const [activities, setActivities] = useState([]);
  
  // Perhatikan: unit otomatis diset ke 'isp'
  const [formData, setFormData] = useState({ id: null, name: '', unit: 'isp', startDate: '', status: 'Open' });
  const[searchTerm, setSearchTerm] = useState('');

  const handleExportExcel = () => {
    // Memaksa ekspor hanya untuk data ISP
  ...
  // --- FETCH DATA DARI BACKEND KHUSUS ISP ---
  const fetchActivities = async () => {
    try {
      const response = await axios.get(API_URL, {
        params: {
          unit: 'isp', // <--- MEMAKSA HANYA AMBIL DATA ISP
          search: searchTerm
        }
      });
  ...
  const handleSaveNew = async () => {
    ...
    try {
      await axios.post(API_URL, {
        ...
        unit: 'isp', // Pastikan tersimpan sebagai ISP
  ...
  const resetForm = () => {
    setFormData({ id: null, name: '', unit: 'isp', startDate: '', status: 'Open' });
    setView('table');
  };
  ...
  if (view === 'form' || view === 'edit') {
    return (
      <div className="h-full flex flex-col items-center justify-center select-none animate-fadeIn">
        <h3 className="mb-8 font-bold text-sm uppercase tracking-widest text-center">MONITORING KEGIATAN (DIVISI ISP)</h3>
        ...
        <div className="bg-[#f3f4f6] border border-black rounded-[40px] p-12 w-full max-w-4xl shadow-sm flex flex-col gap-6">
          <div className="space-y-6">
            ...
            {/* Input Unit Dihilangkan Karena Sudah Pasti ISP */}
            ...
            <div className="flex gap-4">
              ...
              {/* Status hanya muncul saat mode edit, ISP diizinkan merubah ini */}
              {view === 'edit' && (
                <div className="relative flex-1">
                  <select 
    ...
    <div className={`h-full flex flex-col gap-4 select-none transition-all duration-500 ${isExpanded ? 'absolute inset-0 z-50 bg-[#f8f9fa] p-8' : 'relative'}`}>
      ...
      {!isExpanded && (
        <>
          <div className="flex justify-center mb-6">
            <h3 className="font-bold text-xl uppercase tracking-widest">MONITORING KEGIATAN (DIVISI ISP)</h3>
          </div>
          ...
          <div className="flex justify-between items-end mb-4">
            {/* Filter OSP/ISP/ALL dihilangkan sebagai spacer kosong */}
            <div></div>
      ...
      {isExpanded && (
        <div className="relative mb-6">
          ...
          <h3 className="text-center font-bold text-xl uppercase tracking-widest">MONITORING KEGIATAN (DIVISI ISP)</h3>
        </div>
      )}
            ...
            {activities.map((item, index) => (
              <tr key={item.id} className="h-12 border-b border-black hover:bg-gray-50 transition-colors">
                ...
                <td className="border-r border-black p-2">
                  {/* Status Togle untuk ISP, bebas diklik dengan efek hover */}
                  <button 
                    onClick={(e) => { 
                      e.stopPropagation(); 
                ...
                <td className="px-4">
                  {/* Edit & Delete bebas digunakan oleh ISP */}
...
export default ControlKegiatanISP;