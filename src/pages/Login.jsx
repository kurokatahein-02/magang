...
const Login = () => {
  ...
  // Di dalam komponen Login.jsx, sebelum return
  useEffect(() => {
    const token = localStorage.getItem("token");
    if (token) {
      navigate("/dashboard"); // Jika sudah ada token, langsung lempar ke dashboard
    }
  }, [navigate]);
        ...
        <form onSubmit={handleLogin} className="space-y-5">
          ...
          <div className="relative group">
            <div className="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none">
              <User
                size={20}
                className="text-gray-500 group-focus-within:text-[#1a536e] transition-colors"
              />
            </div>
            <input
              required
