...
const ControlSitac = () => {
  ...
  const handleDownload = async (item, e) => {
    ...
    try {
      ...
      // 2. Logika Penamaan File Otomatis
      const date = new Date().toISOString().split("T")[0]; // Ambil YYYY-MM-DD
      const vendorName = item.name.replace(/[^a-z0-9]/gi, "_"); // Bersihkan nama vendor
      const finalFileName = `${vendorName}_SITAC_${date}.pdf`;

      // 3. Eksekusi Download Tanpa Tab Baru
      ...
      console.error("Download error SITAC:", error);
      alert("Gagal mengunduh dokumen SITAC. Pastikan file tersedia di server.");
    }
  };
          ...
          <div className="space-y-4">
                ...
                {formData.lat && (
                  <Marker position={[formData.lat, formData.lng]}>
                    <Tooltip
                      ...
                      direction="top"
                      offset={[0, -10]}
                      className="bg-white border-black font-bold text-[9px] p-1"
                    >
                      LOKASI TERPILIH
                    </Tooltip>
                  </Marker>
            ...
            <div className="relative">
              <input
                ...
                className="w-full p-3.5 rounded-xl border border-black bg-[#d9d9d9] italic px-6 focus:outline-none"
              />
            </div>
            <div className="relative">
              <input
            ...
            <div className="relative">
              <input
                ...
                value={
                  selectedFile
                    ? selectedFile.name
                    : formData.docName || "Masukan Dokumen ...."
                }
                onClick={() => fileInputRef.current.click()}
                className="w-full p-3.5 rounded-xl border border-black bg-[#d9d9d9] italic px-6 focus:outline-none cursor-pointer"
              />
              <input
                type="file"
                ref={fileInputRef}
      ...
      {expandedView === "doc" && (
        <div className="absolute inset-0 z-[60] bg-[#f8f9fa] p-8 animate-fadeIn flex flex-col">
          <button
            ...
            className="mb-4 bg-white border border-black rounded-full w-10 h-10 flex items-center justify-center shadow-md"
          >
            ←
          </button>
          <iframe
            src={selectedDoc?.docUrl}
            className="flex-1 w-full rounded-xl border border-black shadow-2xl"
            title="PDF Viewer"
          />
        </div>
      )}
        ...
        >
          <div className="relative flex items-center justify-center min-h-[40px]">
            {expandedView && (
              <button
                onClick={() => setExpandedView(null)}
                className="absolute left-0 top-1/2 -translate-y-1/2 bg-white border border-black rounded-full w-10 h-10 flex items-center justify-center"
              >
                ←
              </button>
            )}
          ...
          {expandedView !== "table" && (
            <div
              className={`${expandedView === "map" ? "flex-1" : "h-72"} bg-white border border-black rounded-[30px] overflow-hidden transition-all duration-500 shadow-inner relative z-10`}
            >
              {!expandedView && (
                <div
                  ...
                  (v) =>
                    ...
                    v.lng && (
                      <Marker key={v.id} position={[v.lat, v.lng]}>
                        <Popup>
                          <div className="p-1">
                            <h4 className="font-bold text-xs mb-1 uppercase">
                              {v.name}
                            </h4>
                        ...
                        {/* Label Otomatis / Tooltip Permanen */}
                        <Tooltip
                          permanent
                          direction="top"
                          offset={[0, -10]}
                  ...
                  <tbody>
                    {vendors.map((v, i) => (
                      ...
                      >
                        ...
                        <td className="border-r border-black text-[10px] px-3 text-left leading-tight">
                          {v.name}
                        </td>
                        <td className="border-r border-black text-[9px] px-3 text-left leading-tight">
                          {v.location}
                        </td>
                        <td className="border-r border-black text-[10px]">
                          {v.startDate}
                        </td>
                        <td className="border-r border-black text-[10px]">
                          {v.endDate}
