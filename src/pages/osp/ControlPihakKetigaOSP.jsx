        ...
        <div className={`flex flex-col h-full transition-all duration-500 ${expandedView ? 'absolute inset-0 z-50 bg-[#f8f9fa] p-8' : 'gap-6'}`}>
          ...
          {!expandedView && (
            <div className="flex items-center gap-4">
              {!isReadOnly && (
              <button onClick={() => setView('form')} className="bg-[#56a8c7] border border-black rounded-full px-6 py-2 flex items-center gap-2 font-bold text-xs shadow-sm hover:bg-[#4a97b5] transition-all"><Plus size={18} /> Tambah Pihak Ke Tiga</button>
              )}
              </div>
          )}
