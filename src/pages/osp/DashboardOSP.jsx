...
const DashboardOSP = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      // 2. Ambil data KHUSUS inventory OSP untuk dihitung jumlah barangnya
      const responseInv = await axios.get(INVENTORY_API, {
        params: { unit: 'osp' }
      });
      // Set nilai count berdasarkan panjang array data inventory yang ditarik
  ...
  if (loading) return <div className="h-full flex items-center justify-center font-bold animate-pulse text-[#56a8c7] tracking-widest uppercase">Memuat Dashboard OSP...</div>;
      ...
      <div className="flex-1 grid grid-cols-12 gap-6 min-h-0">
        ...
        <div className="col-span-5 flex flex-col gap-6 h-full">
          ...
          >
            ...
            <div className="flex-1 flex flex-col items-center justify-center">
              {/* DISINI MENGGUNAKAN STATE KHUSUS OSP YANG BARU */}
              <div className="text-5xl font-black mb-2 text-black">{ospInventoryCount}</div>
              <div className="text-[10px] uppercase font-bold text-gray-400 tracking-widest">Total Jenis Barang</div>
            </div>
          </div>
        </div>

        {/* KANAN: MAP LOKASI PIHAK KETIGA (Span 7) */}
        <div className="col-span-7 bg-white border border-black rounded-[20px] p-6 flex flex-col shadow-sm h-full">
          <div className="flex items-center justify-center gap-2 mb-4">
            ...
            <div className="text-sm font-black uppercase tracking-[0.2em]">Lokasi Pihak Ketiga</div>
          </div>
          ...
          <div className="flex-1 rounded-2xl overflow-hidden border border-black/10">
              <MapContainer center={[-7.566, 110.831]} zoom={12} style={{ height: '100%', width: '100%' }} key={expanded ? 'expanded-map' : 'mini-map'}>
                  ...
                  {(expanded === 'lokasi-p3' ? data?.markers?.p3 : (data?.markers?.p3 ||[])).map(m => (
                    m.latitude && (
