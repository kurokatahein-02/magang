...
const INVENTORY_API = "http://127.0.0.1:8000/api/inventories"; // Tambahan endpoint inventory

const DashboardOSP = () => {
  const [data, setData] = useState(null);
  const [ospInventoryCount, setOspInventoryCount] = useState(0); // State khusus hitung inventory OSP
  ...
  const fetchData = async () => {
    try {
      ...
      console.error("Dashboard OSP error:", error);
    }
  };
  ...
  if (expanded === "detail-alert-kegiatan") {
    const alertData = (data.alerts?.kegiatan || []).filter(
      (item) => item.unit.toLowerCase() === "osp"
    );

    return (
      <div className="absolute inset-0 z-50 bg-[#f8f9fa] p-8 animate-fadeIn flex flex-col overflow-hidden">
        <HeaderZoom title="Detail Notifikasi Kegiatan OSP" />
            ...
            <div className="mb-6 flex items-center gap-4">
              ...
              <div>
                <h4 className="font-bold text-lg uppercase">
                  Daftar Kegiatan OSP Belum Close (Tunggakan)
                </h4>
                <p className="text-xs text-gray-500 italic">
                  * Menampilkan kegiatan OSP bulan-bulan sebelumnya yang belum diselesaikan.
                </p>
              </div>
            </div>
              ...
              <table className="w-full text-left border-collapse">
                <thead className="bg-[#386097] text-white sticky top-0">
                  <tr>
                    ...
                    <th className="p-4 border-b border-black text-xs font-bold uppercase">Nama Kegiatan</th>
                    <th className="p-4 border-b border-black text-xs font-bold uppercase text-center">Tanggal Mulai</th>
                  ...
                  {alertData.length > 0 ? (
                    alertData.map((item, index) => (
                      <tr key={item.id} className="hover:bg-red-50 transition-colors border-b border-gray-100">
                        ...
                        <td className="p-4 text-xs font-bold uppercase">{item.nama_kegiatan}</td>
                        <td className="p-4 text-xs text-center font-mono text-red-600 font-bold">{item.tanggal_mulai}</td>
                    ...
                    <tr>
                      <td colSpan="4" className="p-10 text-center text-gray-400 italic">Tidak ada tunggakan kegiatan OSP.</td>
                    </tr>
                  )}
                </tbody>
              </table>
  ...
  if (loading)
    return (
      <div className="h-full flex items-center justify-center font-bold animate-pulse text-[#56a8c7] tracking-widest uppercase">
        Memuat Dashboard OSP...
      </div>
    );

  const OSPStats = data?.unitStats?.OSP || {
    open_percent: 0,
    close_percent: 0,
    open_count: 0,
    close_count: 0,
  ...
  const totalKegiatanOSP =
    (OSPStats.open_count || 0) + (OSPStats.close_count || 0);
  // Progres diasumsikan sejajar dengan persentase kegiatan yang sudah 'Close'
  const progressOSP = OSPStats.close_percent || 0;

  // Filter Alert khusus OSP
  const ospAlertCount = (data?.alerts?.kegiatan || []).filter(
    (item) => item.unit.toLowerCase() === "osp"
  ).length;
      ...
      <div className="flex-1 grid grid-cols-12 gap-6 min-h-0">
        ...
        <div className="col-span-5 flex flex-col gap-6 h-full">
          <AlertBox
            ...
            count={ospAlertCount}
            msg="OSP BELUM DISELESAIKAN"
            onClick={() => setExpanded("detail-alert-kegiatan")}
          />

          <DonutBox title="Progres Kegiatan OSP" stats={OSPStats} />
          ...
          >
            ...
            <div className="mt-4 pt-2 border-t border-black/5 flex justify-between items-end">
              <div className="text-[9px] uppercase font-bold text-gray-400">
                {ospInventoryCount} Jenis Barang
              </div>
        ...
        {/* KANAN: MAP LOKASI PIHAK KETIGA DENGAN TOOLTIP IDENTIK ADMIN */}
                ...
                (m) =>
                  ...
                  m.longitude && (
                    <Marker key={m.id} position={[m.latitude, m.longitude]}>
                      {/* Tooltip Permanen (Identik dengan Admin) */}
                      <Tooltip
                        permanent
                        direction="top"
                        offset={[0, -10]}
                      ...
                      <Popup>
                        <div className="font-bold text-xs">{m.nama_vendor}</div>
    ...
    <div className="space-y-1.5 flex-1 flex flex-col justify-center">
      {count > 0 ? (
        ...
        <div className="text-[10px] text-gray-400 italic text-center py-2">
          Tidak ada kegiatan OSP yang menunggak.
        </div>
      )}
    </div>
  </div>
