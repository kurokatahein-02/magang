...
const ControlKegiatanHAI = () => {
  // --- STATE MANAGEMENT ---
  const [view, setView] = useState('table'); 
  const [isExpanded, setIsExpanded] = useState(false);
  const [activities, setActivities] = useState([]);
  
  // Perhatikan: unit otomatis diset ke 'HAI'
  const [formData, setFormData] = useState({ id: null, name: '', unit: 'hai', startDate: '', status: 'Open' });
  const[searchTerm, setSearchTerm] = useState('');

  const handleExportExcel = () => {
    // Memaksa ekspor hanya untuk data HAI
  ...
  // --- FETCH DATA DARI BACKEND KHUSUS HAI ---
  const fetchActivities = async () => {
    try {
      const response = await axios.get(API_URL, {
        params: {
          unit: 'hai', // <--- MEMAKSA HANYA AMBIL DATA HAI
          search: searchTerm
        }
      });
  ...
  const handleSaveNew = async () => {
    ...
    try {
      await axios.post(API_URL, {
        ...
        unit: 'hai', // Pastikan tersimpan sebagai HAI
  ...
  const resetForm = () => {
    setFormData({ id: null, name: '', unit: 'hai', startDate: '', status: 'Open' });
    setView('table');
  };
  ...
  if (view === 'form' || view === 'edit') {
    return (
      <div className="h-full flex flex-col items-center justify-center select-none animate-fadeIn">
        <h3 className="mb-8 font-bold text-sm uppercase tracking-widest text-center">MONITORING KEGIATAN (DIVISI HAI)</h3>
        ...
        <div className="bg-[#f3f4f6] border border-black rounded-[40px] p-12 w-full max-w-4xl shadow-sm flex flex-col gap-6">
          <div className="space-y-6">
            ...
            {/* Input Unit Dihilangkan Karena Sudah Pasti HAI */}
    ...
    <div className={`h-full flex flex-col gap-4 select-none transition-all duration-500 ${isExpanded ? 'absolute inset-0 z-50 bg-[#f8f9fa] p-8' : 'relative'}`}>
      ...
      {!isExpanded && (
        <>
          <div className="flex justify-center mb-6">
            <h3 className="font-bold text-xl uppercase tracking-widest">MONITORING KEGIATAN (DIVISI HAI)</h3>
          </div>
          ...
          <div className="flex justify-between items-end mb-4">
            {/* Filter OSP/HAI/ALL dihilangkan sebagai spacer kosong */}
            <div></div>
      ...
      {isExpanded && (
        <div className="relative mb-6">
          ...
          <h3 className="text-center font-bold text-xl uppercase tracking-widest">MONITORING KEGIATAN (DIVISI HAI)</h3>
        </div>
      )}
        ...
        <table className="w-full text-center border-collapse table-fixed">
          <thead className="bg-[#56a8c7] text-black">
            <tr>
              ...
              {/* Kolom Nama Unit dihapus karena sudah pasti HAI */}
            ...
            {activities.map((item, index) => (
              <tr key={item.id} className="h-12 border-b border-black hover:bg-gray-50 transition-colors">
                ...
                <td className="border-r border-black p-2">
                  {/* Status Togle untuk HAI, bebas diklik dengan efek hover */}
                  <button 
                    onClick={(e) => { 
                      e.stopPropagation(); 
                ...
                <td className="px-4">
                  {/* Edit & Delete bebas digunakan oleh HAI */}
...
export default ControlKegiatanHAI;