...
const ControlKegiatanHAI = () => {
  ...
  // Membuat daftar tahun secara dinamis (Contoh: dari 2020 sampai tahun sekarang)
  ...
  const handleExportExcel = async () => {
    try {
      ...
      const response = await api.get("/activities/export", {
        params: {
          unit: "HAI", // Pastikan export hanya untuk HAI 
          search: searchTerm,
          month: selectedMonth,
          year: selectedYear,
        },
      ...
      const fileName = `Laporan_Kegiatan_HAI ${formattedDate}.xlsx`;
    ...
    return (
      <div className="h-full flex flex-col items-center justify-center select-none animate-fadeIn">
        <h3 className="mb-8 font-bold text-sm uppercase tracking-widest text-center">
          MONITORING KEGIATAN (DIVISI HAI)
        </h3>
      ...
      {!isExpanded && (
        <>
          ...
          <div className="flex justify-between items-end mb-4">
            {/* Filter HAI/HAI/ALL dihilangkan sebagai spacer kosong */}
            <div className="flex gap-4 mb-2">
              <select
                value={selectedMonth}
                onChange={(e) => setSelectedMonth(e.target.value)}
                className="p-2 border border-black rounded-lg text-[10px] font-bold bg-white focus:outline-none"
              >
              ...
              <select
                ...
                className="p-2 border border-black rounded-lg text-[10px] font-bold bg-white focus:outline-none"
              >
                {/* Loop melalui array years yang sudah dibuat di atas */}
                {years.map((year) => (
                ...
                <td className="px-4">
                  ...
                  <div className="flex justify-center gap-4 h-full items-center">
                    <button
                      ...
                      className="bg-[#386097] border text-white rounded-md px-4 py-0.5 text-[9px] font-bold transition-all hover:bg-white "
                    >
                      Edit
                    </button>
                    <button
