...
const InventoryHAI = () => {
  ...
  // State form tanpa 'unit' karena akan di-hardcode ke 'HAI'
  ...
  // --- EXPORT EXCEL KHUSUS HAI ---
  const handleExportExcel = async () => {
    try {
      const response = await api.get("/inventories/export", {
        params: {
          unit: "HAI", // Pastikan export hanya untuk OSP
          search: searchTerm,
        },
      ...
      const fileName = `Inventory_HAI ${formattedDate}.xlsx`;
  ...
  const handleExportHistory = async () => {
    try {
      ...
      const fileName = `Histori_Pengambilan_HAI_${formattedDate}.xlsx`;
  ...
  // --- FETCH DATA KHUSUS HAI ---
  const fetchInventory = async () => {
    try {
      const response = await api.get(API_URL, {
        params: {
          unit: 'HAI', // Paksa hanya narik data HAI
          search: searchTerm
        }
      });
  ...
  if (view === 'form' || view === 'edit') {
    return (
      <div className="h-full flex flex-col items-center justify-center select-none animate-fadeIn">
        <h3 className="mb-6 font-bold text-sm uppercase tracking-widest text-center">INVENTORY CONTROL (DIVISI HAI)</h3>
        <div className="bg-[#f3f4f6] border border-black rounded-[40px] p-12 w-full max-w-5xl shadow-sm flex flex-col gap-6">
          <div className="space-y-6">
            ...
            <div className="flex gap-4">
              ...
              {/* Dropdown Unit Dihapus Karena Pasti HAI */}
      ...
      <div className="h-full flex flex-col gap-6 select-none relative p-4 animate-fadeIn">
        <div className="relative flex items-center justify-center min-h-[40px]">
          ...
          <h3 className="font-bold uppercase tracking-[0.2em] text-xl">
            HISTORI PENGAMBILAN BARANG (HAI)
          </h3>
          <button
  ...
  return (
    <div className={`h-full flex flex-col select-none relative transition-all duration-500 ${isExpanded ? 'absolute inset-0 z-50 bg-[#f8f9fa] p-8' : 'px-4 gap-6'}`}>
      ...
      <div className={`relative flex items-center justify-center ${isExpanded ? 'mb-8' : 'min-h-[40px]'}`}>
        ...
        <h3 className={`font-bold uppercase tracking-[0.2em] ${isExpanded ? 'text-xl' : 'text-sm'}`}>INVENTORY - DIVISI HAI</h3>
      </div>
          ...
          <table className="w-full text-center border-collapse table-fixed">
            <thead className="bg-[#386097]">
              <tr>
                ...
                {/* Kolom Unit Dihapus Karena Semuanya Pasti HAI */}
            ...
            <tbody>
              {inventoryList.map((item, index) => (
                <tr key={item.id} className="h-12 border-b border-black hover:bg-gray-50 transition-colors">
                  ...
                  <td className="px-4">
                    {/* HAI Bebas Ambil, Edit dan Delete */}
