...
const ControlKegiatan = () => {
  ...
  const [formData, setFormData] = useState({ id: null, name: '', unit: '', startDate: '', status: 'Open' });
...
const isSuperAdmin = role === 'superadmin';
  ...
  const handleSaveNew = async () => {
    if(!formData.name || !formData.unit || !formData.startDate) return alert("Isi data terlebih dahulu!");
  ...
  const toggleStatus = async (id) => {
    try {
      // 1. Kirim request PATCH ke backend
      const response = await axios.patch(`${API_URL}/${id}/status`);
      
      if (response.data.success) {
        // 2. Ambil data terbaru dari backend agar tabel terupdate otomatis
        fetchActivities(); 
        
        // Opsi tambahan: Berikan feedback visual kecil
        console.log("Status diperbarui ke: " + response.data.data.status);
      }
    } catch (error) {
  ...
  const resetForm = () => {
    setFormData({ id: null, name: '', unit: '', startDate: '', status: 'Open' });
    setView('table');
  };
  ...
  if (view === 'form' || view === 'edit') {
    return (
      <div className="h-full flex flex-col items-center justify-center select-none animate-fadeIn">
        <h3 className="mb-8 font-bold text-sm uppercase tracking-widest text-center">MONITORING KEGIATAN</h3>
    ...
    <div className={`h-full flex flex-col gap-4 select-none transition-all duration-500 ${isExpanded ? 'absolute inset-0 z-50 bg-[#f8f9fa] p-8' : 'relative'}`}>
      ...
      {!isExpanded && (
        <>
          ...
          <div className="flex justify-center mb-6">
            <h3 className="font-bold text-xl uppercase tracking-widest">MONITORING KEGIATAN</h3>
          </div>
      ...
      {isExpanded && (
        <div className="relative mb-6">
          ...
          <h3 className="text-center font-bold text-xl uppercase tracking-widest">MONITORING KEGIATAN</h3>
        </div>
      )}
        ...
        <table className="w-full text-center border-collapse table-fixed">
          <thead className="bg-[#56a8c7] text-black">
            <tr>
              ...
              <th className="p-3 border-r border-b border-black text-[11px] font-bold">Nama Unit</th>
