...
const Inventory = () => {
  ...
  const [formData, setFormData] = useState({ id: null, name: '', amount: '', unit: '', location: '' });
...
const isSuperAdmin = role === 'superadmin';
  ...
  const handleExportExcel = () => {
    ...
    // 2. Buat URL lengkap ke backend
    // API_URL biasanya: http://127.0.0.1:8000/api/inventories
    const downloadUrl = `${API_URL}/export?${queryParams}`;
    
    // 3. Picu download dengan membuka tab baru sebentar
    window.open(downloadUrl, '_blank');
    
    // 4. Tampilkan notifikasi "Berhasil Diunduh"
  ...
  if (view === 'form' || view === 'edit') {
    return (
      <div className="h-full flex flex-col items-center justify-center select-none animate-fadeIn">
        <h3 className="mb-6 font-bold text-sm uppercase tracking-widest text-center">INVENTORY CONTROL</h3>
        <div className="bg-[#f3f4f6] border border-black rounded-[40px] p-12 w-full max-w-5xl shadow-sm flex flex-col gap-6">
          <div className="space-y-6">
            ...
            <div className="relative w-1/3">
              <select value={formData.unit} onChange={(e) => setFormData({...formData, unit: e.target.value})} className="w-full p-3 rounded-lg border border-black bg-[#d9d9d9] italic px-6 focus:outline-none appearance-none cursor-pointer">
              ...
              <ChevronDown size={18} className="absolute right-4 top-1/2 -translate-y-1/2 text-gray-600" />
            </div>
          ...
          <div className="flex justify-center gap-10 mt-10 font-bold">
            <button onClick={resetForm} className="px-20 py-3 bg-[#d9d9d9] border border-black rounded-lg hover:bg-white transition-all uppercase">KEMBALI</button>
            <button onClick={handleSave} className="px-20 py-3 bg-[#d9d9d9] border border-black rounded-lg hover:bg-white transition-all uppercase">SIMPAN</button>
          </div>
        </div>
      </div>
    );
  ...
  return (
    <div className={`h-full flex flex-col select-none relative transition-all duration-500 ${isExpanded ? 'absolute inset-0 z-50 bg-[#f8f9fa] p-8' : 'px-4 gap-6'}`}>
      ...
      <div className={`relative flex items-center justify-center ${isExpanded ? 'mb-8' : 'min-h-[40px]'}`}>
        ...
        <h3 className={`font-bold uppercase tracking-[0.2em] ${isExpanded ? 'text-xl' : 'text-sm'}`}>INVENTORY - {activeFilter}</h3>
      </div>
              ...
              {['OSP', 'ISP', 'ASO', 'HAI', 'ALL'].map((f) => (
                <button 
                  ...
                  className={`px-6 py-1 border border-black rounded-full font-bold text-[10px] shadow-sm transition-all ${
                    activeFilter === f ? 'bg-[#56a8c7] text-white' : 'bg-white hover:bg-gray-50'
                  }`}
                >
                  {f}
                </button>
          ...
          <table className="w-full text-center border-collapse table-fixed">
            <thead className="bg-[#56a8c7]">
              <tr>
                ...
                <th className="p-3 border-r border-b border-black text-[11px] font-bold">Unit</th>
            ...
            <tbody>
              {inventoryList.map((item, index) => (
                <tr key={item.id} className="h-12 border-b border-black hover:bg-gray-50 transition-colors">
                  ...
                  <td className="border-r border-black text-xs uppercase">{item.unit}</td>
